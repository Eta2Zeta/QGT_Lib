...
def test_square_lattice_harmonics():
    ...
    # Plot 2x2: Real and Imag components
    fig = plt.figure(figsize=(10, 6))

    # Top-left: Real part (analytical)
    ax1 = fig.add_subplot(221, projection='3d')
    ax1.plot_surface(kx, ky, np.real(analytical_H12), cmap='viridis')
    ax1.set_title('Analytical Re(H12)')
    ax1.set_xlabel('$k_x$')
    ax1.set_ylabel('$k_y$')
    ax1.set_zlabel('Re(H12)')

    # Top-right: Real part (numerical)
    ax2 = fig.add_subplot(222, projection='3d')
    ax2.plot_surface(kx, ky, np.real(numerical_H12), cmap='viridis')
    ax2.set_title('Numerical Re(H12)')
    ax2.set_xlabel('$k_x$')
    ax2.set_ylabel('$k_y$')
    ax2.set_zlabel('Re(H12)')

    # Bottom-left: Imag part (analytical)
    ax3 = fig.add_subplot(223, projection='3d')
    ax3.plot_surface(kx, ky, np.imag(analytical_H12), cmap='viridis')
    ax3.set_title('Analytical Im(H12)')
    ax3.set_xlabel('$k_x$')
    ax3.set_ylabel('$k_y$')
    ax3.set_zlabel('Im(H12)')

    # Bottom-right: Imag part (numerical)
    ax4 = fig.add_subplot(224, projection='3d')
    ax4.plot_surface(kx, ky, np.imag(numerical_H12), cmap='viridis')
    ax4.set_title('Numerical Im(H12)')
    ax4.set_xlabel('$k_x$')
    ax4.set_ylabel('$k_y$')
    ax4.set_zlabel('Im(H12)')
