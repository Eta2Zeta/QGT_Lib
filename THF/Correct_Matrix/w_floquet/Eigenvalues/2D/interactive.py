...
z_limit = 200  # Set the z-axis limit
...
def update_plot(G):
    ...
    plt.title(f'Eigenvalues of the Sixth-Order Polynomial Equation along K-Γ-M-K Path (G = {G:.2f})')
    plt.legend()
    plt.grid(True)
    plt.show()

# Create interactive plot with slider for G
interactive_plot = interactive(update_plot, G=widgets.FloatSlider(value=0, min=0, max=0.1, step=0.01, description='G'))
interactive_plot
