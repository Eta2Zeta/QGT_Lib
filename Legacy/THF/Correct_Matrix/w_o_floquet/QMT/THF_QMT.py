...
eigenvalues, eigenfunctions = spiral_eigenvalues_eigenfunctions(H_THF, kx, ky, mesh_spacing, dim)
...
for i in range(kx.shape[0]):
    for j in range(kx.shape[1]):
        ...
        eigenfunction = eigenfunctions[i,j,band]
        eigenvalue = eigenvalues[i,j,band]
        g_xx, g_xy_real, g_xy_imag, g_yy = quantum_geometric_tensor_num(H_THF, kx[i, j], ky[i, j], delta_k, eigenvalue, eigenfunction, dim)
        g_xx_array[i, j] = g_xx
