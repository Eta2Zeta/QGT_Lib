...
from eigenvalue_calc_lib import grid_eigenvalues_eigenfunctions, capping_eigenvalues, eigenvalues_eigenfunctions_in_zone, eigenvalues_eigenfunctions_in_zone_eigenvector_ordering, calculate_neighbor_phase_array
...
# Define parameters
mesh_spacing = 100
k_max = 3 * (np.pi / a)  # Maximum k value for the first Brillouin zone
...
if useExisting:
    ...
    zone_divider = ZoneDivider(kx, ky, num_zones=num_zone, start_angle = np.pi/6 + 0.1)
    ...
    for zone_num in range(1):
        eigenvalues_zone, eigenfunctions_zone, phasefactors_zone, neighbor_phase_array_after_calc = eigenvalues_eigenfunctions_in_zone(
            H_THF, kx, ky, mesh_spacing, dim=6, Zone=zone_divider, zone_num=zone_num, touching_points=touching_points
        )
