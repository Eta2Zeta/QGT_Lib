...
def main():
    ...
    # 2x2 Projected Hamiltonian
    H_proj = ChiralHamiltonianProjected(n=n, vF=vF, t1=t1, V=V)
    ...
    # Projected Hamiltonian has 2 bands
    evals_proj = np.zeros((len(kx_vals), 2))
    ...
    for i, kx in enumerate(kx_vals):
        ...
        # 2. Projected Hamiltonian 2x2
        # We can use the analytic method or the matrix diagonalization.
        # Let's use the matrix diagonalization from compute_static to be consistent with 'Hamiltonian'
        # H_proj_mat = H_proj.compute_static(kx, ky)
        # e_proj = np.linalg.eigvalsh(H_proj_mat)
        # Or use the analytic one provided in the class which might be more exact/faster?
        # The prompt asks for "calculations", usually implies diagonalization but analytic is fine too.
        # Let's use analytic_eigenvalues as it is explicitly available.
        e_proj = H_proj.analytic_eigenvalues(kx, ky)
        evals_proj[i, :] = np.sort(e_proj)
    ...
    ax2 = axes[1]
    # Projected bands
    ax2.plot(kx_vals, evals_proj[:, 0], color='red', linewidth=2, label='Projected -')
    ax2.plot(kx_vals, evals_proj[:, 1], color='red', linewidth=2, linestyle='--', label='Projected +')
    # Add vertical lines
    ax2.axvline(x=k_lim, color='k', linestyle=':', label='Limit')
    ax2.axvline(x=-k_lim, color='k', linestyle=':')
    
    ax2.set_title('Projected Hamiltonian (2x2)')
    ax2.set_xlabel('$k_x$')
    ...
    # Plot projected bands (foreground)
    ax3.plot(kx_vals, evals_proj[:, 0], color='red', linewidth=1.5, linestyle='--', label='Proj')
    ax3.plot(kx_vals, evals_proj[:, 1], color='red', linewidth=1.5, linestyle='--')
    ...
    # The full bandwidth is large, but projected is only valid for low energy.
    ymax = np.max(np.abs(evals_proj)) * 1.5
